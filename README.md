...
Plateforme DevSecOps en microservices pour l’analyse de sécurité des applications Android (MASVS/MSTG) : SAST + secrets + crypto + dynamique réseau + IA (anomalies) + génération de rapports (PDF/JSON/SARIF) + intégration CI/CD.
...
Chaque service est autonome, expose au minimum /health et des routes de scan/résultats, et persiste ses résultats (SQLite + volumes) 

Services principaux (actuels dans le compose)

Service	Rôle	Port hôte → conteneur	Stockage

APKScanner	Analyse statique APK : manifest, permissions, components exportés, flags (debuggable/backup), métadonnées	8001 → 8000	SQLite + data volume
...
CIConnector	Génération de pipelines CI/CD (GitHub Actions / GitLab CI)	8007 → 8000	stateless + accès workspace
...
But : détecter des comportements réseau suspects non couverts par des règles statiques (endpoints rares, patterns atypiques, bursts, etc.) à partir des flows capturés par mitmproxy.
...
threshold_override (ex : 0.05) pour être plus “agressif” et remonter des anomalies.
